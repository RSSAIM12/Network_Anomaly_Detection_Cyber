...
class AlertDashboard:
    ...
    def add_alert(self, alert_info):
        ...
        print(f"🚨 Nouvelle alerte: {alert_data['type']} - Taille: {alert_data['size']} octets")
        return alert_data
...
def process_alert_from_sniffer(alert_info):
    """Fonction pour recevoir les alertes du sniffer"""
    dashboard.add_alert(alert_info)

def start_flask_server():
    """Démarre le serveur Flask"""
    print(f"🚀 Serveur Flask démarré sur http://localhost:{API_PORT}")
...
<body>
    <div class="container">
        ...
        <div class="controls">
            ...
            <button class="btn" onclick="clearAlerts()">
                <i class="fas fa-trash"></i> Effacer Alertes
            </button>
        </div>
    ...
    <script>
        class IDSDashboard {
            ...
            updateAlertsList(alerts) {
                ...
                // Vider la liste sauf si on veut garder l'historique d'animation
            ...
            updateChartsFromAlerts(alerts) {
                ...
                // Timeline simplifiée (groupée par minute)
            ...
            groupAlertsByMinute(alerts) {
                const groups = {};
                alerts.forEach(alert => {
                    const minute = alert.timestamp.substring(0, 16); // YYYY-MM-DD HH:MM
                    groups[minute] = (groups[minute] || 0) + 1;
                });

                const labels = Object.keys(groups).slice(-15); // 15 dernières minutes
